...
class UserModel(Tower):
    ...
    def inference(self):
        ...
        with slim.arg_scope([slim.conv2d, slim.fully_connected],
            ...
            model = slim.conv2d(model, 256, [3, 3], 1, scope='conv4')
            model = slim.max_pool2d(model, [3, 3], 2, scope='pool4')
            model = slim.flatten(model)
            model = slim.fully_connected(model, 512, activation_fn=None, scope='fc1')
            model = slim.dropout(model, 0.5, is_training=self.is_training, scope='do1')
            model = slim.fully_connected(model, 512, activation_fn=None, scope='fc2')
